...
resource "vsphere_virtual_machine" "puppetserver" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/opt/puppetlabs/bin/puppet apply -e \"include roles::puppetserver\" --hiera_config=${var.staging_code_dir}/hiera.yaml --modulepath=${var.staging_code_dir}/site:${var.staging_code_dir}/modules",
