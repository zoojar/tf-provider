...
param (
  ...
  [string]$puppet_agent_certname = $(throw "-puppet_agent_certname is required.")
)
...
$msiexec_args = "/qn /log c:\\windows\\temp\\puppet-enterprise-installer.log /i c:\\windows\\temp\\puppet-enterprise-installer.msi PUPPET_AGENT_CERTNAME=${puppet_agent_certname} PUPPET_MASTER_SERVER=${puppet_master_server}"
